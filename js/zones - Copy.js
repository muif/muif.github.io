...
// ----------------------------------------------------------------------------
// ب. جلب وعرض المناطق لحظياً (Real-time listener)
// ----------------------------------------------------------------------------
onSnapshot(collection(db, "zones"), (snapshot) => {
    ...
    snapshot.forEach((zoneDoc) => {
        ...
        // تغيير الزر ليفتح النافذة المنبثقة بدلاً من الـ Prompt
        ...
        tr.innerHTML = `
            ...
            <td style="padding: 10px; text-align:center;">${zone.has_fat ? '✅' : '❌'}</td>
            <td style="padding: 10px; text-align:center;">${zone.is_active ? '🟢 نشط' : '🔴 معطل'}</td>
...
function attachZoneEvents() {
        ...
        btn.addEventListener('click', async (e) => {
            ...
            if (newFee !== null && newFee.trim() !== "") {
                try {
                    ...
                    alert("تم تحديث أجرة التوصيل بنجاح!");
                } catch (error) {
    ...
    // 2. حدث فتح نافذة إدارة الأعمدة (FAT)
